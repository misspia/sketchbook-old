...
export class Lights {
  constructor() {
    ...
    this.directionalFrontLeft = new THREE.DirectionalLight(0xffffff, 0.8)
    this.directionalFrontLeft.position.set(-1, 0, 1).normalize()

    this.directionalFrontRight = new THREE.DirectionalLight(0xffffff, 0.8)
    this.directionalFrontRight.position.set(1, 0, 1).normalize()

    this.directionalTop = new THREE.DirectionalLight(0xffffff, 1.5)
    this.directionalTop.position.set(0, 1, 0).normalize()

    this.ambient = new THREE.AmbientLight(0x000000, 0.5)
    this.spot = new THREE.SpotLight(0xffffff, 20, 80, 0.5, 1)
